...
def construct_portfolio(
    scored_products: Sequence[Tuple[ProductTypeUnion, Decimal, List[ProductSelectionReason]]],
    ...
    buckets: Dict[str, List[Tuple[ProductTypeUnion, Decimal, List[ProductSelectionReason]]]] = {
        "equity": [],
        "debt": [],
        "gold": [],
        "cash": [],
    ...
    for prod, score, reasons in scored_products:
        asset_str = (
        ...
        if asset_str in buckets:
            buckets[asset_str].append((prod, score, reasons))
        elif asset_str == "hybrid":
            # Hybrid can serve equity or debt buckets if needed
            buckets["equity"].append((prod, score, reasons))
            buckets["debt"].append((prod, score, reasons))
        else:
            buckets["debt"].append((prod, score, reasons))
    ...
    for asset_class, target_pct in target_map.items():
        ...
        if target_pct >= Decimal("50.00") and len(available) >= 2:
            ...
            p1, score1, reasons1 = available[0]
            p2, score2, reasons2 = available[1]
