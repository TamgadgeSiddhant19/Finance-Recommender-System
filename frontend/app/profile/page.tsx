...
export default function ProfilePage() {
  ...
  const calculatedSurplus = Math.max(0, formData.monthly_income - formData.monthly_expenses);
  const calculatedRunway =
    formData.monthly_expenses > 0
      ? (formData.total_savings / formData.monthly_expenses).toFixed(1)
      : "0";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base font-bold text-amber-400">{analysis.risk.risk_category}</p>
            </div>
          </div>
        </div>
