...
## 🔍 Phase 7.4: Recommendation Explainability & Audit Trail

Phase 7.4 makes every recommendation generated by Artha AI **Traceable, Explainable, and Auditable**, establishing institutional-grade transparency without sacrificing speed or determinism.

### Key Capabilities

1. **Deterministic Explanation Object (`RecommendationExplanation`)**:
   - **Risk Factor Breakdown (`RiskExplanation`)**: Itemizes exact point contributions and penalties derived from `calculate_risk_assessment.score_breakdown` (Stated Tolerance 40%, Experience 20%, Age Runway 20%, Emergency Reserve 20%, Debt-to-Income Penalty).
   - **Goal Trajectory (`GoalExplanation`)**: Compounding at 6.0% p.a. inflation, nominal vs. inflated corpus target, expected return, and funding ratio.
   - **Asset Allocation Rationales (`AssetAllocationExplanationItem`)**: Per-asset-class percentage, exact monthly SIP deployment, and governing policy matrix rule.
   - **Verifiable Product Selection Checkmarks (`SelectedProductExplanation`)**: Exact checkmarks explaining why an instrument was selected over alternatives.
   - **Structured Exclusion Taxonomy (`ProductExclusionItem`)**: Separates pre-scoring filters (`HARD_REJECTION`: risk ceiling / ticket size) from scoring outcomes (`RANKED_LOWER`: eligible but outranked).

2. **Immutable Audit Record (`RecommendationAudit`) & Input Snapshot Hash**:
   - Stored in database table `recommendations_audits` with cascade delete.
   - **SHA-256 Input Snapshot Hash**: 64-character hexadecimal hash computed deterministically across financial profile and goal parameters via `compute_input_snapshot_hash(profile, goals)`.
   - **Decision Trace (`DecisionTrace`)**: Complete snapshot of all 26 inputs, intermediate calculations, factor attributions, product screening outcomes, data provenance, and portfolio validation checks.
   - **Zero PII Leakage**: Strict filtering ensures no user passwords, access tokens, API keys, or raw secrets are stored in audit trails.

3. **Protected Audit API Endpoints**:
   - `GET /api/v1/recommendations/me/latest/audit`: Fetches audit log and decision trace for authenticated user's latest recommendation.
   - `GET /api/v1/recommendations/{recommendation_id}/audit`: Fetches audit log with strict user ownership checks (returns HTTP 404/403 for unauthorized access).

4. **Engine & Policy Versioning**:
   - `RECOMMENDATION_ENGINE_VERSION = "7.4.0"`
   - `RECOMMENDATION_POLICY_VERSION = "2026.1"`
